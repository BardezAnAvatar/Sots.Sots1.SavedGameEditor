...
`;
    ...
    for (const instance of ruleData.ruleInstances.sort(compareRuleInstances)) {
        ...
        html += 
            `<tr class="${count == 0 ? "table-active" : ""}" instance-id="${instance.id}" project-path="${instance.projectPath ?? ""}" rule-id="${ruleData.encodedId}">
                <td class="w-75 table-row-wrap-all">
                    <span>${description}</span> <span class="text-info">${position}</span>
                </td>
